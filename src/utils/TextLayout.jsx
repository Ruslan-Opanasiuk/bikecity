...
export function computeTextLayout(params) {
  ...
  // Вимірюємо ширину основного тексту з його фінальним розміром шрифту.
  // Цей вимір повинен бути після всіх можливих зменшень та розбиття на рядки.
  let finalCalculatedWidthMain = 0;
  if (mainTextLines.length === 1) {
    finalCalculatedWidthMain = measureText(mainTextLines[0], `${fontSize1}px RoadUA-Bold`).width;
  } else {
    // Якщо текст розбито на два рядки, беремо ширину найдовшого рядка
    finalCalculatedWidthMain = Math.max(
      measureText(mainTextLines[0], `${fontSize1}px RoadUA-Bold`).width,
      measureText(mainTextLines[1], `${fontSize1}px RoadUA-Bold`).width
    );
  }

  // Прапорець isOverflowing:
  // Текст вважається "переповнюючим", якщо його фінальна ширина
  // більша за доступну ширину мінус буфер.
  const isOverflowing = finalCalculatedWidthMain > (availableTextWidthMain - OVERFLOW_DETECTION_BUFFER);
  ...
  return {
    ...
    waveWidth,
    routeBadgeX,
    isOverflowing, // Додаємо новий прапорець до повертаємого об'єкта
  };
}
